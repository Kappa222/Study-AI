...
| Learning partner | Defaults to Mia (automatically set) — avatar replaces the Leo/Mia partner picker |
...
- SVG logo: `cognimo-wordmark.svg` (Cogni+violet + mo+dark)
...
- Staggered fade-in-up reveal animations on brand panel + form card (matching hero section)

**Subjects & Topics:**

- 3 global subjects (Matematika, Történelem, Irodalom) with logo colors — redesigned cards with gradient top panels, centered full-viewport layout, floating decors
...
- ProgressRoadmap component rendered above tabs when materials exist (7 islands, avatar on current, left/right nav)
...
- Male/female avatar picker (no labels, just images) on `/setup-profile` page — replaces Leo/Mia picker
- Same avatar picker on `/settings` page — replaces tanulótárs section
- Avatar stored in `profiles.avatar_url`
...
- Back buttons on all pages (← Vissza...) — positioned in top-left corner on centered pages

**Component:**

- `ConfirmModal` — reusable modal with backdrop dismiss, focus rings, danger/default variants
- `Header` — auth-aware global header (hidden on `/`, `/login`, `/setup-profile`); sizing unified with landing page hero nav (`text-2xl` logo, `px-5 py-2 text-base` buttons)
- `AnimatedStats` — scroll-triggered count-up with IntersectionObserver + requestAnimationFrame + easeOutExpo
- `ProgressRoadmap` — horizontal island-based progress roadmap (7 islands, avatar on current, left/right arrow nav, 3 phases with tinted backgrounds)
...
- **4c (UI done):** Horizontal progress roadmap with avatar on current island — wired to hardcoded data, awaits session system
...
- ProgressRoadmap component is UI-only — hardcoded to currentCheckpoint=0, awaits session system wiring
...
9. Login page reveal animations, avatar system (male/female SVGs replacing Leo/Mia), ProgressRoadmap component, subjects page redesign, header sizing unified with landing page
